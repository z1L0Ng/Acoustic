...
- Current chapter workflow: the user has resumed the paper-writing task for sequential discussion and revision of Introduction, Datasets/Analysis, Method, and Evaluation. Discuss each chapter directly with the user, complete the agreed edits and red DONE marker, then continue to the next chapter without returning to management for approval. Defer Abstract and Conclusion until the main chapters are settled. The writing and drawing tasks may coordinate directly on figures needed for these chapters; figure work follows the user's chapter discussion. HF-on training runs independently and must not block writing. The user-controlled management-synchronization rule below remains in force.
...
- User-requested review-marker exception: when a section's assigned revision is complete, place one visible red `DONE` at the start of that section, after the heading and before the body. When an assigned figure revision is complete and integrated, place one visible red `DONE` at the start of its figure caption, not inside the PPT/image. Use a shared LaTeX macro with `xcolor` when first needed. Do not mark unfinished work, infer completion from an old draft, or duplicate markers. `DONE` means ready for the user's review, not advisor acceptance. Remove a marker while its item is reopened for unfinished revision, restore it on completion, and do not clear review markers early without the user's request. This explicit marker is permitted despite the general rule against internal manuscript notes; other internal notes remain outside the synchronized manuscript.
