...
CANDIDATE_COMPARISON_CAVEAT = {
    "P1": "AST frontend internally zero-pads each 2 s fbank to the audited 798-frame grid",
    "P2": "BEATs accepts 2 s waveform windows and uses exact valid-patch masking",
    "P3": "official PANNs Cnn14_16k emits 2048-d; comparison includes a trainable shared-across-datasets 2048->768 adapter",
    "P4": "official HeAR Keras serving emits 512-d; comparison includes a trainable shared-across-datasets 512->768 adapter",
...
PRODUCTION_ADAPTER_STATUS = {
    "AST": {
        "status": "code_READY_local_asset_present_CPU_real_smoke_passed",
        ...
        "gate": "L40 CUDA zero-update preflight and independent verifier remain HOLD",
    },
    "BEATs": {
        "status": "code_READY_local_asset_present_CPU_real_smoke_passed",
    ...
    "PANNs_Cnn14": {
        "status": "code_READY_asset_HOLD",
        "required_file": "baseline/multidataset_pipeline/panns_window_encoder.py",
        "gate": "pin local official source/checkpoint SHA and dependency, then real CPU and L40 smoke",
    },
    "HeAR": {
        "status": "code_READY_asset_and_license_acceptance_HOLD",
