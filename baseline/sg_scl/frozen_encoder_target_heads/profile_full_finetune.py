"""One-step SG-SCL full-encoder profile; this does not start a training run."""
...
def main() -> None:
    ...
    cache_root = Path(".cache/sprsound_sg_scl_frozen_encoder_target_heads")
    ...
    images = torch.stack([preprocess_recording([row], repo)[0] for row in rows])
    target = torch.tensor(
    ...
    embeddings = encoder(images, args=inference_args, training=False)
    ...
    if not torch.isfinite(loss) or not all(
        ...
        raise RuntimeError("non-finite SG-SCL full-finetune profile")
    optimizer.step()
