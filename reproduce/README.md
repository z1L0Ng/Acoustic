...
- 统一训练装配：`baseline/multidataset_pipeline/train_shared_window.py`，schema=`shared_window_training_v2`。同一入口支持 P1–P4、明确 optimizer parameter groups、validation-only selection、checkpoint/resume 与 JSON/JSONL receipts。`preflight` 不创建 model/optimizer/run root；`smoke`/`full` 要求绑定 exact config SHA 与 v2 data identity 的 phase-specific approval；resume 同样校验 v2 identity；`terminal-score` 还要求 outer/test approval 与独立 verifier scorer。工程 smoke 不得写成性能结果。
- 新 independent verifier receipt 必须同时核对 provider data identity、manifest/split hashes、runner schema、phase approval、optimizer/trainable scope、resume、checkpoint component hashes、window/adapter receipts、per-dataset/native-task metrics 与 outer/test access；`archived_baseline_verifier` 不可用于本队列，任何 pooled/global score 均非法。
