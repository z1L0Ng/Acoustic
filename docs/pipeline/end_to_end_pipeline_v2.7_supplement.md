# Acoustic End-to-End Pipeline v2.7 — Companion Specification

## 0. Purpose and scope

This document is the audit-oriented companion to:

- `figures/pipeline/acoustic_end_to_end_pipeline_v2.7.svg`
- `figures/pipeline/acoustic_end_to_end_pipeline_v2.7.png`
...
→ unified, native and generalization evaluation
```
...
The v2.7 SVG keeps the ontology crosswalk as a compact supervision-side contract, while the complete mapping and eligibility semantics remain here. The generalization comparison is retained as a scientific evaluation contract in this document but is intentionally omitted from the main SVG.
...
1.0 s stride
short unit → provisional zero padding
```
...
The following are not part of the v2.6 main forward path:
...
2. **Support gate:** Produce per-dataset/per-split eligible counts for every unified node and mask state.
