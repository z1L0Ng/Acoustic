...
3. **低优先：保留`1e-3`并加10轮linear warmup。** 失效发生在30–45轮而非启动阶段，单独warmup的解释力较弱。作者可选MA在每次`optimizer.step()`后把移动平均结果写回当前model/classifier，因此会改变后续训练权重；但它不能修复已经NaN的参数，是否有助稳定、论文实际是否启用均不明确，当前不自动开启。
